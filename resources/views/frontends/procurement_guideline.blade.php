    ...
    <main class="main">
        ...
        <div class="page-title dark-background" data-aos="fade" style="background-color: #2c4666">
            <div class="container position-relative">
                <h1>{{ $sectionSetting?->title ?? __('app.procurement_guidelines') }}
                </h1>
                <nav class="breadcrumbs">
                    <ol>
                        <li><a href="{{ url('/', []) }}" class="text-primary">{{ __('app.home') }}</a></li>
                        <li class="current">
                            {{ $sectionSetting?->title ?? __('app.procurement_guidelines') }}
                        </li>
                    </ol>
                </nav>
            </div>
                                    ...
                                    <form id="downloadGuidelinesForm" method="POST"
                                        ...
                                        @if ($latestProcurementDate)
                                            <div class="alert alert-info alert-dismissible fade show" role="alert">
                                                ...
                                                {{ translate('Latest updated procurement guideline at:', app()->getLocale()) }}
                                            ...
                                            <tbody>
                                                @forelse ($guidelinesProcurement as $guideline)
                                                    <tr>
                                                        ...
                                                        <td width="45%">
                                                            {{ $guideline->category ?? translate('No Data', app()->getLocale()) }}
                                                        </td>
                                                        <td>
                                                            <a href="{{ route('download.uploads', ['file' => $guideline->file_path, 'model' => get_class($guideline), 'id' => $guideline->id]) }}"
                                                                target="_blank" rel="noopener">
                                                                {{ translate(pathinfo(asset($guideline->file_path), PATHINFO_FILENAME), app()->getLocale()) }}
                                                            </a>
                                                        </td>
                                                    </tr>
                                                @empty
