...
@section('title', GoogleTranslate::trans($sectionSetting?->title ?? __('app.Teaching & Leading'), app()->getLocale()))
    ...
    <main class="main">
        ...
        <div class="page-title dark-background" data-aos="fade" style="background-color: #2c4666">
            <div class="container position-relative">
                <h1>{{ GoogleTranslate::trans($sectionSetting?->title ?? __('app.Teaching & Leading'), app()->getLocale()) }}
                </h1>
                <div style="word-wrap: break-word;">
                    {!! GoogleTranslate::trans($sectionSetting?->description, app()->getLocale()) !!}
                </div>
                <nav class="breadcrumbs">
                    <ol>
                        <li><a href="{{ url('/', []) }}" class="text-primary">{{ __('app.home') }}</a></li>
                        <li class="current">
                            {{ GoogleTranslate::trans($sectionSetting?->title ?? __('app.Teaching & Leading'), app()->getLocale()) }}
                        </li>
                    </ol>
                </nav>
            </div>
                        ...
                        <div class="row gx-5">
                            @foreach ($news as $post)
                                <div class="project_single col-md-4">
                                    ...
                                    @endif
                                    <h3><a
                                            href="{{ route('what-we-do.teaching-leadings-detail', ['locale' => session('locale', 'en'), 'id' => $post->id]) }}">{{ GoogleTranslate::trans($post->title, app()->getLocale()) }}</a>
                                    </h3>
