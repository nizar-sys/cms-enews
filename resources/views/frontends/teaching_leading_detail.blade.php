    ...
    <main class="main">
        ...
        <div class="page-title dark-background" data-aos="fade" style="background-color: #2c4666">
            <div class="container position-relative">
                <h1>{{ $sectionSetting?->title ?? __('app.Teaching & Leading') }}
                </h1>
                <nav class="breadcrumbs">
                    <ol>
                        <li><a href="{{ url('/', []) }}" class="text-primary">{{ __('app.home') }}</a></li>
                        <li class="current">
                            {{ $sectionSetting?->title ?? __('app.Teaching & Leading') }}
                        </li>
                    </ol>
                </nav>
            </div>
                            ...
                            <footer class="entry-footer">
                                @if ($news->file)
                                    <div class="m-4">
                                        ...
                                        {{ translate('File Attachment', app()->getLocale()) }}: <br>
                                        <a href="{{ route('download.uploads', ['file' => $news->file, 'model' => get_class($news), 'id' => $news->id]) }}"
                                            target="_blank"
                                            class="btn btn-primary btn-sm mt-2">{{ translate('Download', app()->getLocale()) }}</a>
                                    </div>
                                @endif
                            </footer><!-- .entry-footer -->
                        </article><!-- #post-## -->
                                ...
                                @foreach ($latestNews as $latestNew)
                                    <li class="news_li">
                                        <a class="newspost-title"
                                            href="{{ route('what-we-do.teaching-leadings-detail', ['locale' => session('locale', 'en'), 'id' => $latestNew->id]) }}">{{ $latestNew->title }}</a>
                                    </li>
                                @endforeach
